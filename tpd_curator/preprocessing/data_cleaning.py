...
def clean_processed_json_data(
    ...
    if (
        sections_to_remove
        and "sections" in cleaned_data
        and isinstance(cleaned_data["sections"], dict)
    ):
    ...
    if (
        figure_fields_to_remove
        and "figures" in cleaned_data
        and isinstance(cleaned_data["figures"], list)
    ):
...
def clean_single_json_file(
    ...
    try:
        ...
        cleaned_data = clean_processed_json_data(
            data, fields_to_remove, sections_to_remove, figure_fields_to_remove
        )
...
def clean_json_data(
    ...
    sections_to_remove: Optional[List[str]] = None,
    figure_fields_to_remove: Optional[List[str]] = None,
    paper_filter_file: Optional[Union[str, Path]] = None,
) -> List[Dict[str, Any]]:
    """Clean JSON data and return list of cleaned data dictionaries for pipeline use"""
    if not fields_to_remove:
        raise ValueError(
            "fields_to_remove must be provided and cannot be empty. Please specify json_fields_to_remove in your config file."
        )
    ...
    print("Created timestamped directory:")
    ...
    for i, json_file in enumerate(json_files, start=1):
        ...
        try:
            ...
            cleaned_json = clean_processed_json_data(
                data, fields_to_remove, sections_to_remove, figure_fields_to_remove
            )
...
def save_cleaned_json_data(
    cleaned_data: List[Dict[str, Any]],
    output_dir: Union[str, Path],
    file_prefix: str = "cleaned",
) -> List[Path]:
