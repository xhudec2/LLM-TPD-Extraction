...
def clean_pdf_data(
    ...
    convert_pdf_to_md(
        config_parser, str(pdf_path), str(output_dir), save_images=save_images
    )
...
def clean_pmc_data(
    ...
    if pmc_path.is_file() and pmc_path.name == "processed.json":
        ...
        raise ValueError(
            f"Invalid PMC path. Expected directory or processed.json file: {pmc_path}"
        )
    ...
    for json_file in processed_files:
        ...
        try:
            ...
            cleaned_pmc = {
                k: v for k, v in pmc_data.items() if k not in fields_to_remove
            }
    ...
    if data_type is None:
        if input_path.is_file():
            ...
            if list(input_path.rglob("processed.json")):
                ...
                raise ValueError(
                    f"No PDF or processed.json files found in: {input_path}"
                )
