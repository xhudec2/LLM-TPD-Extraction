...
try:
    ...
    print(
        "Warning: python-docx is not available. DOCX file processing will be disabled."
    )
...
def find_latest_cleaned_json_dir(output_dir: Path) -> str:
    ...
    if not cleaned_papers_dir.exists():
        raise FileNotFoundError(
            f"Cleaned papers directory does not exist: {cleaned_papers_dir}"
        )
    ...
    if not timestamp_dirs:
        raise FileNotFoundError(
            f"No timestamped directories found in {cleaned_papers_dir}"
        )
    ...
    if not latest_cleaned_dir.exists():
        raise FileNotFoundError(
            f"Cleaned JSONs directory does not exist: {latest_cleaned_dir}"
        )
...
def _read_text_head(
    file_path: Union[str, Path], n=5, encodings=("utf-8-sig", "utf-8", "latin1")
) -> Tuple[str, str]:
...
def _try_multiheader_read_csv(file_path, encoding, delimiter):
    ...
    if isinstance(df.columns, pd.MultiIndex):
        flatten_bad = sum(
            1
            for lvl in df.columns.levels
            for v in lvl
            if isinstance(v, str) and v.startswith("Unnamed")
        )
...
def _coerce_numeric(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in out.columns:
        ...
        american_thousand = s2.str.contains(
            r"\d,\d{3}(?!\d)", regex=True, na=False
        ).any()
        ...
        if file_ext == ".csv":
            ...
            for sheet_name in sheet_names:
                try:
                    try:
                        df = pd.read_excel(
                            file_path, sheet_name=sheet_name, header=[0, 1], dtype=str
                        )
                        if isinstance(df.columns, pd.MultiIndex):
                            flatten_bad = sum(
                                1
                        ...
                        df = pd.read_excel(
                            file_path, sheet_name=sheet_name, header=0, dtype=str
                        )
                    ...
                    sheets_content.append(
                        {"name": sheet_name, "content": sheet_content}
                    )
...
def _filter_excel_sheets_by_keywords(file_path: Union[str, Path]) -> str:
    ...
    try:
        ...
        if not relevant_sheets:
            if filtered_sheets:
                print(
                    f"    All {len(filtered_sheets)} sheets filtered out (no keywords)"
                )
            return ""
...
def load_supplementary_files_for_pmc(
    pmc_id: str, json_path: str, filter_by_keywords: bool = True
) -> List[Dict[str, str]]:
    ...
    try:
        for file_path in supplementary_folder.iterdir():
            ...
            if file_ext == ".md":
                ...
                if content:
                    ...
                    print(
                        f"  ✓ Including {file_path.name} (markdown - for IUPAC name extraction)"
                    )
                ...
                if content:
                    if filter_by_keywords:
                        if _contains_relevant_keywords(content):
                            ...
                            print(
                                f"  ✓ Including {file_path.name} (contains DC50/Dmax)"
                            )
                        else:
                            files_filtered_out += 1
                            print(
                                f"  ✗ Filtering out {file_path.name} (no relevant keywords found)"
                            )
                    else:
                        supplementary_files.append(
                            {
                ...
                if content:
                    ...
                    print(
                        f"  ✓ Including {file_path.name} (docx - for IUPAC name extraction)"
                    )
            else:
                continue
        ...
        if not supplementary_files:
            if files_filtered_out > 0:
                print(
                    f"No relevant supplementary files found for {pmc_id} (filtered out {files_filtered_out} files/sheets without keywords)"
                )
            else:
        ...
        if filter_by_keywords and files_filtered_out > 0:
            print(
                f"Filtered out {files_filtered_out} files/sheets without relevant keywords"
            )
...
def record_llm_input_characters(
    tracking_data: dict,
    round_number: int,
    round_type: str,
    description: str,
    ...
    paper_text: str = None,
    supplementary_filename: str = None,
) -> None:
    ...
    print(
        f"  Character stats - Total: {total_chars}, Prompt: {prompt_chars}, Paper: {paper_chars}"
    )
...
def save_character_length_json(tracking_data: dict, output_path: Path) -> None:
    ...
    try:
        ...
        print(f"\n{'=' * 60}")
        print("Character Length Statistics Summary:")
        print(f"  Total rounds: {tracking_data['summary']['total_rounds_counted']}")
        print(
            f"  Total characters: {tracking_data['summary']['total_characters_all_rounds']:,}"
        )
        print(
            f"  Total prompt characters: {tracking_data['summary']['total_prompt_characters']:,}"
        )
        print(
            f"  Total paper characters: {tracking_data['summary']['total_paper_characters']:,}"
        )
...
def test_history_memory(session_id: str):
    ...
    for i, msg in enumerate(history.messages):
        ...
        print(
            f"  {i + 1:02d}. {role}: {msg.content[:80]}{'...' if len(msg.content) > 80 else ''}"
        )
...
def clean_json_papers_batch(config: dict):
    ...
    if not json_path or not Path(json_path).exists():
        raise FileNotFoundError(
            f"JSON data path does not exist or not specified: {json_path}"
        )
...
def run_llm_pipeline(
    ...
    if read_papers:
        ...
        if not json_files:
            ...
            return
        print(
            f"Running LLM reasoning on {len(json_files)} files in {json_file_dir} ..."
        )
        ...
        for json_file in json_files:
            ...
            tracking_data = initialize_character_tracking(
                pmc_id=file_number, with_history=with_history if with_history else False
            )
            ...
            if with_history:
                ...
                first_round_prompt = construct_prompt(
                    custom_prompt=prompt_text, paper_text=file_content
                )
                ...
                if config["data"].get("include_supplementary", False):
                    ...
                    filter_by_keywords = config["data"].get(
                        "filter_supplementary_by_keywords", True
                    )
                    ...
                    if filter_by_keywords:
                        ...
                        print(
                            "Loading all supplementary files without keyword filtering..."
                        )
                    ...
                    if supplementary_files:
                        ...
                        for idx, supp_file in enumerate(supplementary_files, 1):
                            ...
                            if file_type in ["markdown", "docx"]:
                                file_type_display = (
                                    "markdown" if file_type == "markdown" else "Word"
                                )
...
If no additional data is found in this file, return an empty array: []"""
                            ...
                            third_round_prompt = construct_prompt(
                                custom_prompt=supp_custom_prompt, paper_text=content
                            )
                            ...
                            try:
                                ...
                                if records_supp:
                                    if isinstance(records_supp, str):
                                        print(
                                            f"Warning: LLM returned text instead of structured JSON for {filename}"
                                        )
                                        print(
                                            "This may indicate the file contains no relevant data or LLM failed to parse."
                                        )

                                        text_out = (
                                            file_paper_dir
                                            / f"{file_number}_supplementary_{idx}_{filename}_text_response.txt"
                                        )
                        ...
                        if all_supplementary_results:
                            out3 = (
                                file_paper_dir
                                / f"{file_number}_supplementary_extraction.json"
                            )
                            ...
                            print(
                                f"Total supplementary files processed: {len(all_supplementary_results)}"
                            )
                        ...
                        print(
                            "Reviewing main text to find missing IUPAC names and SMILES for extracted compounds..."
                        )
...
If no additional IUPAC names or SMILES are found, return an empty array: []"""
                        ...
                        try:
                            ...
                            if isinstance(records_final_review, list):
                                ...
                                if records_final_review:
                                    print(
                                        f"Final review found {len(records_final_review)} compounds with updated IUPAC names/SMILES"
                                    )
                                else:
                                    print(
                                        "No additional IUPAC names or SMILES found in main text"
                                    )
                            else:
                                with out_final.open("w", encoding="utf-8") as f:
                        ...
                        print(
                            f"No supplementary files found for {file_number}, skipping supplementary rounds."
                        )
                else:
        ...
        if llm_output:
            output_json_file = results_dir / "answer.json"
            try:
                with open(output_json_file, "w", encoding="utf-8") as out_f:
...
def save_csv(config_file: dict, results_dir: str = None):
    ...
    if results_dir:
        ...
        if not latest_json_dir.exists():
            raise ValueError(
                f"Specified results directory does not exist: {results_dir}"
            )
        ...
        if timestamped_dirs:
            ...
            print(
                f"Loading JSON files from the latest directory (by timestamp): {latest_json_dir}"
            )
        else:
            latest_json_dir = max(all_subdirs, key=lambda d: d.stat().st_mtime)
            print(
                f"Loading JSON files from the latest directory (by modification time): {latest_json_dir}"
            )
    ...
    if paper_ids_file:
        ...
        molglue_papers_path = Path(
            "/Users/yaochenr/project/tpd_curator/data_source/250825_molglue_papers/paper_dois/avaliable_paper_ids.json"
        )
    ...
    for json_file in latest_json_dir.rglob("*.json"):
        ...
        if not (
            json_file.name.endswith("_extraction.json")
            or json_file.name.endswith("_supplementary_extraction.json")
            or json_file.name.endswith("_final_review.json")
        ):
                ...
                if filename.endswith("_supplementary_extraction"):
                    ...
                    if isinstance(content, list):
                        for supp_file in content:
                            if (
                                isinstance(supp_file, dict)
                                and "extraction" in supp_file
                            ):
                                ...
                                if isinstance(extraction, list):
                                    for record in extraction:
                                        if isinstance(record, dict):
                                            record["DOI"] = doi
                                            record["source"] = (
                                                f"supplementary_{supp_file.get('filename', 'unknown')}"
                                            )
...
def convert_pdc50_to_nm(value_str: str) -> str:
    ...
    if (
        value_str is None
        or (isinstance(value_str, float) and pd.isna(value_str))
        or value_str == "null"
    ):
        return value_str
...
def process_units(csv_file_path: str):
    ...
    try:
                    ...
                    if pd.notna(row["DC50"]) and row["DC50"] != "null":
                        ...
                        if is_pdc50_unit(unit) or is_pdc50_unit(dc50_value):
                            ...
                            if converted_value != dc50_value:
                                ...
                                print(
                                    f"  Converted: {dc50_value} {unit} → {converted_value} nM (pDC50)"
                                )
                            else:
                                print(
                                    f"  Warning: Could not convert pDC50 value {dc50_value}"
                                )
                            continue
                        ...
                        if has_embedded_unit or unit.lower() not in ["nm", "nanomolar"]:
                            if unit.lower() in ["nm", "nanomolar"]:
                                ...
                                if cleaned_value != dc50_value:
                                    ...
                                    print(
                                        f"  Cleaned: {dc50_value} → {cleaned_value} (unit: nM)"
                                    )
                            else:
                                ...
                                if converted_value != dc50_value:
                                    ...
                                    df.at[idx, "DC50_units"] = "nM"
                                    print(
                                        f"  Converted: {dc50_value} {unit} → {converted_value} nM"
                                    )
                                else:
                                    cleaned_value = clean_dc50_value(dc50_value)
                                    if cleaned_value != dc50_value:
                                        df.at[idx, "DC50"] = cleaned_value
                                        print(
                                            f"  Warning: Could not convert unit {unit}, but cleaned: {dc50_value} → {cleaned_value}"
                                        )
                                    else:
                                        print(
                                            f"  Warning: Could not convert {dc50_value} {unit} to nM"
                                        )
        ...
        if len(key_fields) < 2:
            print(
                f"Warning: Not enough key fields for deduplication (found: {key_fields}). Skipping deduplication."
            )
            key_fields = None
        else:
        ...
        if len(identity_fields) >= 2 and len(value_fields) >= 1:
            print("Running subsumption deduplication...")
            ...
            df["_sub_key"] = (
                df[identity_fields].fillna("").astype(str).agg("|".join, axis=1)
            )
            rows_to_drop = set()
            ...
            print(
                f"  After subsumption dedup: {len(df)} records (removed {before_sub - len(df)})\n"
            )
        else:
        ...
        if os.path.exists(cache_file_path):
            with open(cache_file_path, "r", encoding="utf-8") as f:
                ...
                if isinstance(cache_data, dict):
                    ...
                    print(
                        f"Warning: Cache file {cache_file_path} is in legacy format, migrating to DOI-grouped format"
                    )
                    return {"_legacy": cache_data}
                else:
                    print(
                        f"Warning: Cache file {cache_file_path} format is invalid, starting with empty cache"
                    )
                    return {}
        else:
            print(
                f"Cache file {cache_file_path} does not exist, starting with empty cache"
            )
            return {}
    except Exception as e:
        print(
            f"Error loading cache file {cache_file_path}: {e}, starting with empty cache"
        )
        return {}
...
def find_cache_match(
    ...
    def search_in_list(items: List[Dict]) -> str:
        ...
        for item in items:
            ...
            if (
                cache_pred.lower() == pred_value.lower()
                and cache_labeled.lower() == labeled_value.lower()
                and item.get("feature", "") == feature
            ):
                return item["result"]

            if (
                cache_pred.lower() == labeled_value.lower()
                and cache_labeled.lower() == pred_value.lower()
                and item.get("feature", "") == feature
            ):
                return item["result"]
        return None
...
def check_semantic_match(
    ...
    cached_result = find_cache_match(
        pred_value, labeled_value, feature, cache_data, doi=doi
    )
    if cached_result is not None:
        print(
            f"Cache hit! The comparison result for {pred_value} and {labeled_value} ({feature}) is: {cached_result}"
        )
    ...
    for _ in range(3):
        try:
            ...
            if result and isinstance(result, str):
                ...
                print(
                    f"Added to cache ({cache_key}): The comparison result for {pred_value} and {labeled_value} ({feature}) is {result_text}"
                )

                return is_match
            else:
                print(
                    f"Cannot parse LLM's result: {result}, continue with exact matching."
                )
...
def calculate_assay_match_score(
    pred_row, ref_row, assay_fields: list = None, tiebreak_fields: list = None
) -> int:
...
def score_stereoisomer_match(
    pred_row,
    gt_row,
    error_margin: float = _STEREO_ERROR_MARGIN,
    tiebreak_fields: list = None,
) -> int:
...
def _classify_fp_type(
    pred_compound: str, attempted_matches: List[Dict], gt_compounds_in_paper: Set[str]
) -> str:
    ...
    if not attempted_matches:
        ...
        for gt_comp in gt_compounds_in_paper:
            ...
            if (
                ...
                or pred_lower.replace(" ", "") == gt_lower.replace(" ", "")
            ):
...
def _generate_fp_summary(
    fp_type: str,
    pred_row: pd.Series,
    attempted_matches: List[Dict],
    gt_compounds_in_paper: Set[str],
) -> Dict:
    ...
    if fp_type == "compound_not_in_labeled_data":
        ...
        for attempt in attempted_matches:
            for field in attempt.get("mismatch_fields", []):
                mismatch_fields_counter[field] = (
                    mismatch_fields_counter.get(field, 0) + 1
                )
        ...
        if len(gt_values) == 1:
            ...
            gt_values_str = (
                f"[{', '.join(gt_values[:3])}, ... ({len(gt_values)} total)]"
            )
...
def evaluate_record_based_accuracy(
    pred_csv_path: str,
    ...
    output_dir: str = None,
    ternary_complex_level: bool = False,
) -> dict:
    ...
    try:
        if cache_file_path is None:
            if (
                config
                and "evaluation" in config
                and "cache_file_path" in config["evaluation"]
            ):
            ...
            if paper_filter_file and Path(paper_filter_file).exists():
                ...
                try:
                    ...
                    if filter_dois:
                        ...
                        print("Will filter ground truth to only include these DOIs")
                        ...
                        print(
                            f"  Filtered out: {gt_records_before - gt_records_after} records from {len(gt_dois_before) - len(gt_dois_after)} DOIs"
                        )
                except Exception as e:
        ...
        print(f"\n{'=' * 60}")
        print("DOI Filtering (Predictions):")
        ...
        print(
            f"  Filtered out: {records_before - records_after} records from {len(pred_dois_before) - len(pred_dois_after)} DOIs"
        )
        print(f"{'=' * 60}")
        ...
        for doi in labeled_df["DOI"].unique():
            ...
            gt_compounds_by_doi[doi] = set(
                doi_labeled_df["Compound_Name"].dropna().unique()
            )
        ...
        for doi in labeled_df["DOI"].unique():
            ...
            detailed_results_by_doi[str(doi)] = {
                ...
                "part2_field_level": {"matched_records": []},
            }
            ...
            for pred_idx, pred_row in doi_pred_df.iterrows():
                pred_compound = (
                    pred_row["Compound_Name"] if "Compound_Name" in pred_row else None
                )
                pred_conn_key_raw = (
                    pred_row["Connectivity_Key"]
                    if "Connectivity_Key" in pred_row
                    else None
                )
                has_compound_name = not pd.isna(pred_compound)
                has_conn_key = pd.notna(pred_conn_key_raw) and str(
                    pred_conn_key_raw
                ).strip().lower() not in ("", "nan")

                if not has_compound_name and not has_conn_key:
                    print(
                        f"Warning: Skipping pred row {pred_idx} with missing Compound_Name and Connectivity_Key"
                    )
                    continue
                ...
                if has_compound_name:
                    ...
                    matching_labeled = doi_labeled_df[
                        doi_labeled_df["Compound_Name"].apply(
                            lambda x: normalize_dashes(str(x).lower())
                        )
                        == pred_compound_norm
                    ]
                else:
                ...
                if (
                    matching_labeled.empty
                    and has_conn_key
                    and "Connectivity_Key" in doi_labeled_df.columns
                ):
                    pred_conn_key = normalize_dashes(
                        str(pred_conn_key_raw).strip().lower()
                    )
                    matching_labeled = doi_labeled_df[
                        doi_labeled_df["Connectivity_Key"].apply(
                            lambda x: normalize_dashes(str(x).lower())
                        )
                        == pred_conn_key
                    ]
                ...
                if matching_labeled.empty:
                    ...
                    fp_summary = _generate_fp_summary(
                        fp_type, pred_row, [], gt_compounds
                    )
                    fp_record["fp_summary"] = fp_summary

                    detailed_results_by_doi[str(doi)]["part1_record_level"][
                        "fp_records"
                    ].append(fp_record)
                ...
                for label_idx, label_row in matching_labeled.iterrows():
                    ...
                    if assay_cell_match:
                        candidates.append(
                            (label_idx, label_row, ref_has_empty_assay_or_cell)
                        )
                    else:
                        attempted_matches.append(mismatch_info)
            ...
            for label_idx, competing_pred_indices in ref_to_competing_preds.items():
                ...
                available_competing_preds = [
                    p
                    for p in competing_pred_indices
                ...
                if best_pred_idx is not None:
                    ...
                    matched_record_id = (
                        label_row["Record_ID"] if "Record_ID" in label_row else None
                    )
            ...
            for pred_idx, pred_info in pred_candidates_map.items():
                ...
                if len(available_candidates) == 0:
                    ...
                    gt_compounds = gt_compounds_by_doi.get(doi, set())
                    fp_type = _classify_fp_type(
                        str(pred_compound), attempted_matches, gt_compounds
                    )
                    fp_summary = _generate_fp_summary(
                        fp_type, pred_row, attempted_matches, gt_compounds
                    )
                    fp_record["fp_summary"] = fp_summary
                    ...
                    if found_match:
                        ...
                        tp_record = {
                            ...
                            "label_compound": str(label_row.get("Compound_Name", "")),
                            "label_target": str(
                                label_row.get("Degradation_Target", "")
                            ),
                            "label_recruiter": str(label_row.get("Recruiter", "")),
                            "label_assay": str(label_row.get("Assay", "")),
                            "label_cell_line": str(label_row.get("Cell_Line", "")),
                    ...
                    pred_matched_by_conn_key = pred_info.get(
                        "matched_by_conn_key", False
                    )
                    ...
                    if pred_matched_by_conn_key:
                        ...
                        if max_score > 0:
                            ...
                            tp_record = {
                                "pred_compound": str(pred_compound),
                                "pred_target": str(
                                    pred_row.get("Degradation_Target", "")
                                ),
                                "pred_recruiter": str(pred_row.get("Recruiter", "")),
                                "pred_assay": str(pred_row.get("Assay", "")),
                                "pred_cell_line": str(pred_row.get("Cell_Line", "")),
                        ...
                        if best_candidate is None:
                            best_candidate = (
                                available_candidates[0][0],
                                available_candidates[0][1],
                            )
            ...
            if doi_pred_df.empty:
                ...
                predicted_compounds = set(
                    doi_pred_df["Compound_Name"].apply(
                        lambda x: normalize_dashes(str(x).lower())
                    )
                )
            ...
            for label_idx, label_row in doi_labeled_df.iterrows():
                ...
                compound_in_pred = (
                    normalize_dashes(str(label_compound).lower()) in predicted_compounds
                )
        ...
        precision_records_micro = (
            total_tp / (total_tp + total_fp) if (total_tp + total_fp) > 0 else 0
        )
        recall_records_micro = (
        ...
        f1_records_micro = (
            ...
            * precision_records_micro
            * recall_records_micro
            / (precision_records_micro + recall_records_micro)
            if (precision_records_micro + recall_records_micro) > 0
            else 0
        )
        ...
        precision_records = (
            sum(per_doi_precisions) / len(per_doi_precisions)
            if per_doi_precisions
            else 0
        )
        recall_records = (
            sum(per_doi_recalls) / len(per_doi_recalls) if per_doi_recalls else 0
        )
        ...
        for matched_record in part1_results["matched_records"]:
            ...
            if str(doi) not in part2_results["per_doi_field_metrics"]:
                ...
                for field in assay_fields:
                    part2_results["per_doi_field_metrics"][str(doi)]["per_field"][
                        field
                    ] = {"tp": 0, "fp": 0, "fn": 0, "tn": 0}
            ...
            for field in assay_fields:
                ...
                if field in ("Assay", "Cell_Line"):
                    entry_match = (
                        ...
                        if not (pred_is_missing or label_is_missing)
                        else None
                    )
                elif ternary_complex_level and field == "DC50_units":
                    entry_match = (
                        compare_concentration_units(pred_val, label_val)
                        if not (pred_is_missing or label_is_missing)
                        else None
                    )
                else:
                    entry_match = (
                        compare_assay_values(pred_val, label_val)
                        if not (pred_is_missing or label_is_missing)
                        else None
                    )
            ...
            detailed_results_by_doi[str(doi)]["part2_field_level"][
                "matched_records"
            ].append(record_fields_detail)
        ...
        precision_entry_micro = (
            total_tp_entry / (total_tp_entry + total_fp_entry)
            if (total_tp_entry + total_fp_entry) > 0
            else 0
        )
        recall_entry_micro = (
            total_tp_entry / (total_tp_entry + total_fn_entry)
            if (total_tp_entry + total_fn_entry) > 0
            else 0
        )
        f1_entry_micro = (
            2
            * precision_entry_micro
            * recall_entry_micro
            / (precision_entry_micro + recall_entry_micro)
            if (precision_entry_micro + recall_entry_micro) > 0
            else 0
        )
        ...
        for field in assay_fields:
            ...
            field_recall = (
                field_tp / (field_tp + field_fn) if (field_tp + field_fn) > 0 else 0
            )
            field_f1 = (
            ...
            part2_results["per_field_metrics"][field]["precision"] = float(
                field_precision
            )
        ...
        precision_entry = (
            sum(per_doi_entry_precisions) / len(per_doi_entry_precisions)
            if per_doi_entry_precisions
            else 0
        )
        recall_entry = (
            sum(per_doi_entry_recalls) / len(per_doi_entry_recalls)
            if per_doi_entry_recalls
            else 0
        )
        f1_entry = (
            sum(per_doi_entry_f1s) / len(per_doi_entry_f1s) if per_doi_entry_f1s else 0
        )
        ...
        print(
            f"Detailed evaluation results (with per-record labels) saved to: {detailed_results_path}"
        )
...
def run_evaluation(
    ...
    if not pred_csv_path:
        ...
        all_subdirs = [
            d
            for d in Path(json_file_path).iterdir()
            if d.is_dir()
            and len(d.name) == 11
            and d.name[:6].isdigit()
            and d.name[7:11].isdigit()
        ]
        if not all_subdirs:
    ...
    if not labeled_csv_path:
        if (
            "evaluation" in config_file
            and "labeled_data_path" in config_file["evaluation"]
        ):
            labeled_csv_path = config_file["evaluation"]["labeled_data_path"]
        else:
            print(
                "No labeled data path specified. Please provide labeled_csv_path or update the config."
            )
            return None
...
def _match_compounds(row_a, row_b) -> str:
    ...
    if not a_name_missing and not b_name_missing:
        if normalize_dashes(str(a_name).strip().lower()) == normalize_dashes(
            str(b_name).strip().lower()
        ):
...
def _no_field_contradictions(
    row_a, row_b, fields, config, cache_file_path, doi
) -> bool:
...
def _all_fields_positively_match(
    row_a, row_b, fields, config, cache_file_path, doi
) -> bool:
...
def _cascade_match_records(doi, baseline_df, llm_df, config, cache_file_path) -> dict:
    ...
    def _greedy_assign(candidates):
        ...
        candidates_sorted = sorted(candidates, key=lambda c: (-c[2], c[0], c[1]))
    ...
    for bl_idx, bl_rec in bl_active.items():
        for llm_idx, llm_rec in llm_active.items():
            ...
            if not _identity_via_assay_fingerprint(bl_rec, llm_rec):
                continue
            if not _no_field_contradictions(
                bl_rec, llm_rec, L1_FIELDS, config, cache_file_path, doi
            ):
                continue
            if not _all_fields_positively_match(
                bl_rec, llm_rec, L3_FIELDS, config, cache_file_path, doi
            ):
                continue
            if _all_fields_positively_match(
                bl_rec, llm_rec, L1_FIELDS, config, cache_file_path, doi
            ):
                level = 1
            elif _all_fields_positively_match(
                bl_rec, llm_rec, L2_FIELDS, config, cache_file_path, doi
            ):
                level = 2
            else:
                level = 3
    ...
    for level in (1, 2, 3):
        active_cands = [
            c
            for c in fallback_buckets[level]
...
def run_llm_vs_baseline_evaluation(
    config_file: dict,
    ...
    if not baseline_csv_path:
        if (
            "evaluation" in config_file
            and "baseline_data_path" in config_file["evaluation"]
        ):
            baseline_csv_path = config_file["evaluation"]["baseline_data_path"]
        else:
            print(
                "No baseline data path specified. Please provide --baseline or set evaluation.baseline_data_path in config."
            )
            return None
    ...
    if (
        config_file
        and "evaluation" in config_file
        and "cache_file_path" in config_file["evaluation"]
    ):
    ...
    for doi in sorted(all_dois):
        ...
        result = _cascade_match_records(
            doi, doi_baseline, doi_llm, config_file, cache_file_path
        )
    ...
    total_matched = (
        global_counts["intersection_l1"]
        + global_counts["intersection_l2"]
        + global_counts["intersection_l3"]
    )
    ...
    print("\n  Sanity check:")
    print(
        f"    Matched + Baseline_Only = {total_matched + global_counts['baseline_only']}  (should be {total_baseline})"
    )
    print(
        f"    Matched + LLM_Only      = {total_matched + global_counts['llm_only']}  (should be {total_llm})"
    )
                    ...
                    if isinstance(result, dict):
                        ...
                        if structures or properties:
                            props = {
                                ...
                                "num_ro5_violations": properties.get(
                                    "num_ro5_violations"
                                ),
...
def fetch_smiles_multi_source(
    csv_file_path: str,
    use_chembl: bool = True,
    rate_limit_pubchem: float = 0.2,
    rate_limit_chembl: float = 0.5,
) -> Path:
    ...
    for idx, row in df.iterrows():
        ...
        try:
            query_display = (
                query_name if len(query_name) <= 50 else query_name[:47] + "..."
            )
            ...
            if compounds and len(compounds) > 0:
                ...
                df.at[idx, "Canonical_SMILES"] = getattr(
                    compound, "canonical_smiles", None
                )
                df.at[idx, "InChI"] = getattr(compound, "inchi", None)
                df.at[idx, "InChIKey"] = getattr(compound, "inchikey", None)
                df.at[idx, "Molecular_Formula"] = getattr(
                    compound, "molecular_formula", None
                )
                df.at[idx, "Molecular_Weight"] = getattr(
                ...
                df.at[idx, "HBond_Donor_Count"] = getattr(
                    compound, "h_bond_donor_count", None
                )
                df.at[idx, "HBond_Acceptor_Count"] = getattr(
                    compound, "h_bond_acceptor_count", None
                )
                df.at[idx, "Rotatable_Bond_Count"] = getattr(
                    compound, "rotatable_bond_count", None
                )
                df.at[idx, "Exact_Mass"] = getattr(compound, "exact_mass", None)
                df.at[idx, "Monoisotopic_Mass"] = getattr(
                    compound, "monoisotopic_mass", None
                )

                if (
                    not pd.notna(df.at[idx, "IUPAC_Name"])
                    or not str(df.at[idx, "IUPAC_Name"]).strip()
                ):
    ...
    for idx in failed_rows:
        ...
        if status == "success" and props:
            if not pd.notna(df.at[idx, "Canonical_SMILES"]) and props.get(
                "canonical_smiles"
            ):
            ...
            if not pd.notna(df.at[idx, "Molecular_Formula"]) and props.get(
                "molecular_formula"
            ):
                df.at[idx, "Molecular_Formula"] = props.get("molecular_formula")

            if not pd.notna(df.at[idx, "Molecular_Weight"]) and props.get(
                "molecular_weight"
            ):
    ...
    if chembl_failed:
        failed_log_path_final = (
            csv_path.parent / "failed_smiles_lookup_after_chembl.json"
        )
